...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json({ error: "RentCast API error", details: errorText }, { status: 500 });
        }
        const data = await res.json();
        return NextResponse.json({ demo: false, property: data });
    } catch (e) {
        console.error('Fetch failed:', e);
        return NextResponse.json({ error: "Failed to fetch property data", details: String(e) }, { status: 500 });
    }
} 