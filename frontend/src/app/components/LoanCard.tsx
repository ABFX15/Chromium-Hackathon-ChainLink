...
export function LoanCard({ loan }: LoanCardProps) {
  ...
  const handleRepayLoan = async () => {
    ...
    try {
      const success = await repayLoan(loan.loanId);
  ...
  const healthFactor = (loan as any).healthFactor || 0;
  const isLowHealthFactor = healthFactor < 1.5;
  const propertyName =
    (loan as any).propertyName || `Property #${loan.tokenId.toString()}`;

  return (
    <div
      className={`bg-gradient-to-br from-black/90 to-cyan-950/20 border p-4 font-mono text-sm rounded-lg backdrop-blur-sm ${
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-cyan-500/20 pb-3 mb-4">
        ...
        <span
          className={`text-xs px-3 py-1 border rounded-lg transition-colors duration-200 ${
            loan.isActive
      ...
      <div className="space-y-3 text-xs">
        <div className="text-cyan-300 font-bold text-sm">{propertyName}</div>
        <div className="text-cyan-500/70">
          token_id: {loan.tokenId.toString()}
        </div>
        <div className="text-sm text-gray-400">
          {propertyName} Value: $
          {(loan as any).propertyValue?.toLocaleString() || "Unknown"}
        </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`font-semibold ${
                isLowHealthFactor ? "text-red-300" : "text-cyan-300"
              }`}
            >
        ...
        <div className="pt-3">
          ...
          <div className="w-full bg-cyan-900/30 h-2 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(
                  parseFloat(getHealthFactorWidth(healthFactor)),
                  100
                )}%`,
              }}
            />
